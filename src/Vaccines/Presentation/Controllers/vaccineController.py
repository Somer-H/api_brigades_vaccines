...
def createVaccineController (vaccine: VaccineBaseScheme, db: Session = Depends(get_db)) -> VaccineScheme: 
...
def getVaccinesController (db: Session = Depends(get_db)) -> list[VaccineScheme]: 
    return getVaccinesService(db)
def getVaccineByIdController (id: int, db: Session = Depends(get_db)) -> VaccineScheme: 
    if not id: 
...
def editVaccineController (id: int, vaccine: VaccineEditScheme, db: Session = Depends(get_db)) -> VaccineScheme: 
    if not id: 
        raise HTTPException(status_code=400, detail="Debe ingresar una ID")
    if id <= 0: 
...
def deleteVaccineController (id: int, db: Session = Depends(get_db)) -> str: 
    if not id: 
        raise HTTPException(status_code=400, detail="Debe ingresar una ID")
    if id <= 0: 
        raise HTTPException(status_code=400, detail="La ID no debe ser menor o igual a 0")
